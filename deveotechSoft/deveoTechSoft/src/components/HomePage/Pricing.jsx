    ...
    <div className="w-full bg-gray-50 py-12 px-4 md:px-16">
      ...
      <div className="flex justify-center gap-4 mb-8 flex-wrap">
        {Object.keys(planData).map((type) => (
          <button
            ...
            onClick={() => {
              setActivePlan(type);
              setCenterIndex(0); // reset center
              sliderRef.current?.slickGoTo(0); // reset position
            }}
      ...
      <Slider {...settings} ref={sliderRef}>
        {cards.map((card, index) => {
          const isCenter = index === centerIndex % cards.length;
          const scaleClass = isCenter ? 'scale-105' : 'scale-95';
          const heightClass = isCenter ? 'h-[400px]' : 'h-[340px]';
